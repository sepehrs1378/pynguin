...
def test_function_different_callables_different_hashes(default_test_case, function_mock, type_system):
    ...
    assert not other_statement.structural_eq(function_statement, {other_statement.ret_val: other_statement.ret_val})
    assert function_statement.structural_hash({function_statement.ret_val: 0}) != other_statement.structural_hash({
        other_statement.ret_val: 0
    })


def test_constructor_statement_eq_other_type(test_case_mock, variable_reference_mock, constructor_mock):
    statement = stmt.ConstructorStatement(test_case_mock, constructor_mock)
...
def test_constructor_mutate_parameter_choose_none(constructor_mock, default_test_case):
    ...
    with mock.patch("pynguin.testcase.testfactory.is_optional_parameter") as optional_mock:
        ...
        assert const._mutate_parameter(
            "a",
            MagicMock(parameters={"a": default_test_case.test_cluster.type_system.convert_type_hint(float)}),
        )
        assert isinstance(
...
def test_constructor_param_count_of_type(default_test_case, constructor_mock):
    ...
    assert const._param_count_of_type(default_test_case.test_cluster.type_system.convert_type_hint(float)) == 1
...
def test_method_statement_not_eq(default_test_case, method_mock):
    ...
    assert not statement.structural_eq(statement2, {statement.ret_val: statement2.ret_val, var1: var1, var2: var2})
...
def test_method_statement_eq(default_test_case, method_mock):
    ...
    assert statement.structural_eq(statement2, {statement.ret_val: statement2.ret_val, var1: var1, var2: var2})
    assert statement.structural_hash({var1: 0, statement.ret_val: 1}) == statement2.structural_hash({
        var1: 0,
        statement2.ret_val: 1,
    })
...
def test_method_mutate_special_parameters_no_mutation(default_test_case, method_mock, variable_reference_mock):
    meth = stmt.MethodStatement(default_test_case, method_mock, variable_reference_mock)
    assert not meth._mutate_special_parameters(0.0)


def test_method_mutate_special_parameters_none_found(method_mock, constructor_mock, default_test_case):
    float0 = stmt.FloatPrimitiveStatement(default_test_case, 5.0)
...
def test_method_mutate_special_parameters_one_found(method_mock, constructor_mock, default_test_case):
    float0 = stmt.FloatPrimitiveStatement(default_test_case, 5.0)
    const0 = stmt.ConstructorStatement(default_test_case, constructor_mock, {"a": float0.ret_val})
