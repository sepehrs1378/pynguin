...
def test_entered_function(simple_module, tracer_mock):
    ...
    simple_module.simple_function.__code__ = transformer.instrument_module(simple_module.simple_function.__code__)
...
def test_add_cmp_predicate(simple_module, tracer_mock):
    adapter = BranchCoverageInstrumentation(tracer_mock)
    transformer = InstrumentationTransformer(tracer_mock, [adapter])
    simple_module.cmp_predicate.__code__ = transformer.instrument_module(simple_module.cmp_predicate.__code__)
...
def test_add_cmp_predicate_loop_comprehension(simple_module, tracer_mock):
    adapter = BranchCoverageInstrumentation(tracer_mock)
    transformer = InstrumentationTransformer(tracer_mock, [adapter])
    simple_module.comprehension.__code__ = transformer.instrument_module(simple_module.comprehension.__code__)
    call_count = 5
...
def test_add_cmp_predicate_lambda(simple_module, tracer_mock):
    adapter = BranchCoverageInstrumentation(tracer_mock)
    transformer = InstrumentationTransformer(tracer_mock, [adapter])
    simple_module.lambda_func.__code__ = transformer.instrument_module(simple_module.lambda_func.__code__)
...
def test_integrate_branch_distance_instrumentation(
    ...
    assert len(tracer.get_subject_properties().branch_less_code_objects) == branchless_function_count
...
def test_offset_calculation_checked_coverage_instrumentation(simple_module):
    ...
    for expected_instr, actual_instr in zip(expected_executed_instructions, trace.executed_instructions, strict=False):
...
def test_jump_if_true_or_pop():
    ...
    def func(string, _int_type=int):
        return (hasattr(string, "is_integer") or hasattr(string, "__array__")) or (isinstance(string, bytes | str))
...
def test_tracking_covered_statements_explicit_return(simple_module):
    ...
    assert tracer.lineids_to_linenos(tracer.get_trace().covered_line_ids) == OrderedSet([77, 78])
...
def test_tracking_covered_statements_while_loop(simple_module, number, expected_lines):
    ...
    adapter = LineCoverageInstrumentation(tracer)
    transformer = InstrumentationTransformer(tracer, [adapter])
    simple_module.while_loop.__code__ = transformer.instrument_module(simple_module.while_loop.__code__)
    tracer.current_thread_identifier = threading.current_thread().ident
