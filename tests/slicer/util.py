...
def compare(dynamic_slice: list[UniqueInstruction], expected_slice: list[Instr]):
    ...
    if len(expected_copy) != 0:
        raise ValueError("Expected slice has remaining instructions:", expected_copy)  # pragma: no cover
    if len(slice_copy) != 0:
...
def _contains_name_argtype(instr_list: list[Instr], unique_instr: UniqueInstruction) -> Instr | None:
    for instr in instr_list:
