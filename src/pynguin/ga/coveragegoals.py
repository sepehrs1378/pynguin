...
class AbstractBranchCoverageGoal(AbstractCoverageGoal):
    ...
    def __init__(
        ...
        *,
        ...
        """
        super().__init__(code_object_id)
        assert is_branchless_code_object ^ is_branch, "Must be either branch-less code object or branch."
        self._is_branchless_code_object = is_branchless_code_object
        self._is_branch = is_branch

    @abstractmethod
    def get_distance(self, result: ExecutionResult, tracer: ExecutionTracer) -> cfd.ControlFlowDistance:
...
class BranchGoal(AbstractBranchCoverageGoal):
    ...
    def get_distance(  # noqa: D102
        ...
        return cfd.get_non_root_control_flow_distance(result, self._predicate_id, self._value, tracer)

    def is_covered(self, result: ExecutionResult) -> bool:  # noqa: D102
        trace = result.execution_trace
        distances = trace.true_distances if self._value else trace.false_distances
        return self._predicate_id in trace.executed_predicates and abs(distances[self._predicate_id]) < 0.00001
...
class BranchGoalPool:
    ...
    def __init__(self, subject_properties: SubjectProperties):  # noqa: D107
        self._branchless_code_object_goals = self._compute_branchless_code_object_goals(subject_properties)
    ...
    def _compute_branchless_code_object_goals(
        ...
        return [
            BranchlessCodeObjectGoal(code_object_id) for code_object_id in subject_properties.branch_less_code_objects
        ]
